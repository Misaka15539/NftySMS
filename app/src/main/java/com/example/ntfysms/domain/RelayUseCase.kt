package com.example.ntfysms.domain
...
class RelayUseCase @Inject constructor(
    ...
    suspend fun testConnection(): Boolean {
        ...
        val result = ntfyRepository.post(
            topicUrl = topicUrl,
            title = context.getString(R.string.relay_test_title),
            body = context.getString(R.string.relay_test_body),
            auth = authConfig,
        ...
        logRepository.insert(
            LogEntry(
                timestampMs = System.currentTimeMillis(),
                sender = context.getString(R.string.relay_sender_system),
                messagePreview = context.getString(R.string.relay_preview_test),
    ...
    private suspend fun performRelay(sender: String, body: String, logId: Long?): Boolean {
        ...
        val result = ntfyRepository.post(
            topicUrl = topicUrl,
            title = context.getString(R.string.relay_notification_title, sender),
            body = body,
            auth = authConfig,
            sslValidation = sslValidationEnabled,
        )
        ...
        if (!sslValidationEnabled && (authConfig !is AuthConfig.None)) {
            logRepository.insert(
                LogEntry(
                    ...
                    detail = context.getString(R.string.relay_ssl_warning_detail),
                )
            )
        }
    ...
    private fun RelayResult.toLogOutcomeAndDetail(): Pair<LogOutcome, String?> = when (this) {
        is RelayResult.Success -> LogOutcome.SUCCESS to context.getString(R.string.relay_http_status, httpStatus)
        is RelayResult.HttpError -> LogOutcome.HTTP_ERROR to context.getString(R.string.relay_http_status, httpStatus)
